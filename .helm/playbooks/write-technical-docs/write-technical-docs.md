...
- Outline the section headings. For large docs (>500 lines likely), outline sections, save the outline using `access-working-store` (the host's own memory/scratch mechanism), and plan to produce linked section files rather than one monolith (see Large Document Protocol below).
...
1. **Outline first.** Save the full section outline using `access-working-store` (the host's own memory/scratch mechanism) before writing any content.
...
3. **Checkpoint between sections.** Update the working-store checkpoint after each completed section with: target folder, full outline, filenames of completed sections, key decisions (terminology choices, structural decisions, scope changes).
