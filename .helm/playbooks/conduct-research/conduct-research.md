...
1. **Retrieve and document each source individually** – fetch one URL with `fetch-url`, write up the key findings from that source as a brief inline note (title, main claim, relevant details, confidence tier), then move to the next. Do not batch-fetch multiple URLs in parallel.
...
Target total report length: 800–1,200 words. Per-section targets:

- **Executive Summary** — 50–100 words
- **Key Findings** — 400–600 words total across all findings
- **What Most People Miss** — 150–250 words
- **Recommendations** — 100–200 words

Exceed these only when the question genuinely demands it. A 700-word report that answers the question is better than a 1,400-word report that buries it.
