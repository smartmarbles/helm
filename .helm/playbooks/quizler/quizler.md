...
**Write permissions:** the working register described below (`temp_question_register`, held in the host's own memory/scratch mechanism via `access-working-store`) · `artifacts/docs/DEFINITIONS.md` only  
...
**You do not call other agents at any point.** QUIZ never invokes another agent — this is a hard constraint regardless of host. If a step would require an agent call, it is a scope violation. Work from file inspection and conversation only.
...
**Never use a dropdown/option-select variant of `ask-user`.** That interaction style strips freeform reasoning from answers. Freeform text answers are the source of definition candidates and ADR candidates — structured selections destroy this signal.
...
The `temp_question_register` is a working file you maintain during a session, held via `access-working-store` in the host's own memory/scratch mechanism. It is not a durable artifact and must never be treated as one.

**Location:** the host's own memory/scratch mechanism (session-scoped working state)  
**Scope:** current session only — do not assume it persists across sessions  
...
Overwrite the working register (`access-working-store`) with an empty register containing only the schema header row. Any data from a prior invocation is discarded at this step.
...
Overwrite the working register (`access-working-store`) with an empty register containing only the schema header row. Any data from a prior invocation is discarded.
...
Overwrite the working register (`access-working-store`) with an empty register containing only the schema header row.
