...
**Scope:** Open-question protocol applies to any generated document with open questions. Approval gates remain checkpoint-only: full path has Spec and Plan gates; standard path has the Plan gate only. Open questions can also surface a second way — mid-task, from inside a dispatched agent's own result, rather than from a generated document. See § Return-and-Relay below for that trigger.
...
### Return-and-Relay (mid-task trigger)

**This is the second way a human checkpoint can trigger, alongside the document-triggered Spec and Plan Checkpoints below — not a replacement for either.** The Spec and Plan Checkpoints fire when a generated *document* contains open questions. Return-and-relay fires *mid-task*, when a dispatched agent hits a blocking unknown partway through its work, after that work has already started. A dispatched agent never prompts the user itself — it returns its open question(s) in its structured result to whichever agent dispatched it, and that agent relays them onward.

**This extends QUIZ's clarification role; it is not a parallel or competing mechanism.** QUIZ remains the standard pre-engagement clarification path — run before a subagent's work begins, against a generated document, per the Open-question protocol above. Return-and-relay is what happens when a blocking unknown surfaces later, mid-task, when there is no clean document checkpoint to route through.

**The open-questions-block shape.** Every open question a subagent returns must carry:
- a **question ID** — unique within that agent's returned result (e.g., `Q1`, `Q2`);
- the **verbatim question text** — the relaying agent must not paraphrase or summarize it;
- **options and/or a default**, where applicable;
- a **blocking vs. deferrable classification** — blocking questions must be answered before the dependent work can proceed; deferrable questions may be recorded and answered later without stalling the task.

**Stateless redispatch.** Dispatched agents are stateless — a subagent cannot be "resumed" by handing it just the answer. When a blocking question is answered, the ORIGINAL dispatching agent must redispatch the subagent with the answer folded into a fresh, complete brief that covers the full task again — never a follow-up message referencing the earlier dispatch.

**Bubble-upward rule.** If a nested subagent (an agent that itself dispatches another agent) raises an open question, that question propagates upward through every calling layer to the ultimate orchestrator/user. An intermediate layer must relay the question onward — it must never silently resolve, absorb, or answer it on the user's behalf.
