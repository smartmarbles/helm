...
- **Plan Gate — `approve` required before first dispatch.** For Standard and Full Path requests, no implementing agent may be dispatched until the user's response contains the literal word `approve`. Semantic equivalents ("sounds good", "go ahead", "yes", "do it", "just build it") do NOT satisfy the gate. This keyword requirement applies only to the initial spec/plan gate — the checkpoint that authorizes the first implementing agent dispatch against a new spec or plan. Phase-to-phase progression within an already-approved plan does NOT require `approve`; semantic cues ("proceed", "continue", "next") are valid for advancing phases. The Research Path (SCOOP → QUILL) does not require a plan gate.
