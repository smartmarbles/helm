...
def _extract_agents(description):
    """Extract known agent names mentioned in the description."""
    found = [name for name in sorted(_KNOWN_AGENTS) if name in description]
    return ", ".join(found) if found else "\u2014"


def _write_roster(results, roster_path, skills_base_dir):
    """Write the skills roster markdown file from validation results."""
    ...
    for r in results:
        skill = r["skill"]
        desc = ""
        skill_md = os.path.join(skills_base_dir, skill, "SKILL.md")
        ...
        used_by = _extract_agents(desc)
        warn_count = str(len(r["warnings"]))
...
def main():
    ...
    parser.add_argument("--update-roster", action="store_true", help="Generate .github/skills-roster.md (requires --all)")
    parser.add_argument("--roster-path", default=None, help="Override roster output path (default: derived from path)")
    args = parser.parse_args()

    if args.update_roster and not args.all:
        print("ERROR: --update-roster requires --all", file=sys.stderr)
        sys.exit(2)
    ...
    if args.update_roster:
        ...
        if not roster_path:
            roster_path = os.path.join(os.path.dirname(os.path.normpath(args.path)), "skills-roster.md")
        _write_roster(results, roster_path, os.path.normpath(args.path))
        print(f"Roster written to {roster_path}", file=sys.stderr)
